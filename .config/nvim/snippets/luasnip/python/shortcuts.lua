...
-- stylua: ignore start
return {
	s({trig = 'main', desc = "if __name__ == '__main__'"}, fmt([[
		if __name__ == '__main__':
			{}
	]], { util.selection(1, 'SELECT_DEDENT', 'pass') })),

	s({trig = 'rni', desc = 'raise NotImplementedError'}, {
		t 'raise NotImplementedError'
	}),

	s({trig = 'pprint', desc = 'Quick and dirty pprint'}, fmt("__import__('pprint').pprint({})", i(1, 'expr'))),
}, { -- Autosnippets
}
-- stylua: ignore end
